...
class ViewPainelPrincipal(discord.ui.View):
    ...
    async def preview_entrada(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_embed(cfg["join_title"], body, cfg["join_color"],
            cfg["join_banner"] or FFZ_THUMBNAIL, build_footer(cfg, interaction.guild),
    ...
    async def preview_saida(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_embed(cfg["leave_title"], body, cfg["leave_color"],
            cfg["leave_banner"] or FFZ_THUMBNAIL, build_footer(cfg, interaction.guild),
            interaction.user.display_avatar.url)
        ...
        try:
            ...
            if inviter_id and cfg.get("antifake_enabled") and joined_at:
                ...
                if tempo_no_servidor < horas_min:
                    ...
                    # Atualiza ranking ao vivo imediatamente após detectar fake
                    if member.guild.id in _ranking_live:
                        ch_id, msg_id = _ranking_live[member.guild.id]
                        try:
                            canal_live = member.guild.get_channel(ch_id)
                            if canal_live:
                                cfg_live = await get_config(member.guild.id)
                                _, tp    = await get_ranking_data(member.guild, 1)
                                embed_r  = await build_ranking_embed(member.guild, cfg_live, 1)
                                view_r   = ViewRanking(member.guild, cfg_live, 1, tp, aba="ranking")
                                msg_live = await canal_live.fetch_message(msg_id)
                                await msg_live.edit(embed=embed_r, view=view_r)
                        except Exception as e:
                            print(f"[RANKING_LIVE] Erro ao atualizar após fake: {e}")
            ...
            # Atualiza ranking ao vivo quando qualquer membro sai (afeta posições)
            if not fake and member.guild.id in _ranking_live:
                ch_id, msg_id = _ranking_live[member.guild.id]
                try:
                    canal_live = member.guild.get_channel(ch_id)
                    if canal_live:
                    ...
                    print(f"[RANKING_LIVE] Erro ao atualizar após saída: {e}")
