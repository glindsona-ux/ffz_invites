...
def build_embed(title: str, body: str, color_hex: str, banner: str,
                footer: str, thumbnail_url: str = None) -> discord.Embed:
    embed = discord.Embed(
        description=body,
...
class ModalMensagemEntrada(discord.ui.Modal, title="✏️ Mensagem de Entrada"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await set_config(
            ...
            join_title  = self.join_title.value,
            join_body   = self.join_body.value,
            join_color  = self.join_color.value or "5865F2",
            join_banner = self.join_banner.value,
        )
        await interaction.response.send_message(
            "✅ Mensagem de **entrada** atualizada!", ephemeral=True
        )
...
class ModalMensagemSaida(discord.ui.Modal, title="✏️ Mensagem de Saída"):
    ...
    leave_body = discord.ui.TextInput(
        label="Conteúdo (use {username} {inviter})",
        style=discord.TextStyle.paragraph,
        placeholder="👤 **Membro**\n`{username}`\n\n🎯 **Foi recrutado por**\n{inviter}",
        max_length=1000,
    )
    leave_color = discord.ui.TextInput(
        label="Cor da embed (hex sem #)",
        placeholder="e74c3c",
        max_length=6,
        required=False,
    )
    leave_banner = discord.ui.TextInput(
        label="URL do banner (opcional)",
        placeholder="https://i.imgur.com/...",
        required=False,
        max_length=300,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await set_config(
            interaction.guild_id,
            leave_title  = self.leave_title.value,
            leave_body   = self.leave_body.value,
            leave_color  = self.leave_color.value or "e74c3c",
            leave_banner = self.leave_banner.value,
        )
        await interaction.response.send_message(
            "✅ Mensagem de **saída** atualizada!", ephemeral=True
        )


class ModalEmojis(discord.ui.Modal, title="😀 Personalizar Emojis"):
    emoji_join = discord.ui.TextInput(
        label="Emoji de Entrada", placeholder="🔵 ou <:nome:ID>", max_length=50
    )
    emoji_leave = discord.ui.TextInput(
        label="Emoji de Saída", placeholder="😔 ou <:nome:ID>", max_length=50
    )
    emoji_inviter = discord.ui.TextInput(
        label="Emoji de Recrutador", placeholder="🎯 ou <:nome:ID>", max_length=50
    )
    emoji_stats = discord.ui.TextInput(
        label="Emoji de Estatísticas", placeholder="📊 ou <:nome:ID>", max_length=50
    )
    emoji_member = discord.ui.TextInput(
        label="Emoji de Membro", placeholder="👤 ou <:nome:ID>", max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await set_config(
            interaction.guild_id,
            emoji_join    = self.emoji_join.value,
            emoji_leave   = self.emoji_leave.value,
            emoji_inviter = self.emoji_inviter.value,
            emoji_stats   = self.emoji_stats.value,
            emoji_member  = self.emoji_member.value,
        )
        await interaction.response.send_message(
            "✅ Emojis atualizados!", ephemeral=True
        )
...
class SelectCanalEntrada(discord.ui.ChannelSelect):
    def __init__(self):
        super().__init__(
            placeholder="📥 Selecione o canal de ENTRADA",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ Canal de **entrada** definido para {canal.mention}", ephemeral=True
        )
...
class SelectCanalSaida(discord.ui.ChannelSelect):
    def __init__(self):
        super().__init__(
            placeholder="📤 Selecione o canal de SAÍDA",
            channel_types=[discord.ChannelType.text],
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ Canal de **saída** definido para {canal.mention}", ephemeral=True
        )
...
class ViewPainelPrincipal(discord.ui.View):
    ...
    async def preview_entrada(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_embed(
            cfg["join_title"], body, cfg["join_color"],
    ...
    async def preview_saida(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_embed(
            cfg["leave_title"], body, cfg["leave_color"],
            cfg["leave_banner"] or FFZ_THUMBNAIL,
            build_footer(cfg, interaction.guild),
            interaction.user.display_avatar.url,
        )
        await interaction.response.send_message(
            content="👁️ **Preview da mensagem de saída:**", embed=embed, ephemeral=True
        )
...
async def setup(bot: commands.Bot):
    ...
    async def on_ready():
        ...
        try:
            ...
            print(f"[CONVITE] Erro ao sincronizar slash commands: {e}")
        ...
        if log_channel_id:
            ...
            if canal_log:
                ...
                embed_log.set_author(
                    name=f"{cfg['emoji_join']} ENTRADA — {member.name}",
                    icon_url=member.display_avatar.url
                )
                embed_log.set_thumbnail(url=member.display_avatar.url)
                embed_log.add_field(
                    name=f"{cfg['emoji_member']} Membro",
                    value=f"{member.mention}\n`{member.name}`\n`ID: {member.id}`",
                    inline=True,
                )
                embed_log.add_field(
                    name=f"{cfg['emoji_inviter']} Recrutado por",
                    value=inviter_info,
                    inline=True,
                )
                embed_log.add_field(
                    name=f"{cfg['emoji_stats']} Convites do recrutador",
                    value=f"`{total_convites}`",
                    inline=True,
                )
                embed_log.add_field(
    ...
    async def on_member_remove(member: discord.Member):
        ...
        inviter_obj     = member.guild.get_member(inviter_id) if inviter_id else None
        inviter_mention = inviter_obj.mention if inviter_obj else (
            f"`ID: {inviter_id}`" if inviter_id else "`Não identificado`"
        )

        # ─ Mensagem no canal de SAÍDA ─
        ...
        if log_channel_id:
            ...
            if canal_log:
                ...
                embed_log.set_author(
                    name=f"{cfg['emoji_leave']} SAÍDA — {member.name}",
                    icon_url=member.display_avatar.url
                )
                embed_log.set_thumbnail(url=member.display_avatar.url)
                embed_log.add_field(
                ...
                embed_log.add_field(
                    name=f"{cfg['emoji_inviter']} Foi recrutado por",
